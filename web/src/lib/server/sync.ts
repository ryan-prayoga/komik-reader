...
export type SyncResult = {
	...
	// Reject client clocks more than 15 minutes ahead.
	const MAX_CLOCK_SKEW_MS = 15 * 60 * 1000;

	export type SyncApplyResult = SyncResult & {
		/** Highest updatedAt among changes the server actually accepted this request. */
...
export function syncChanges(
		...
		const now = Date.now();
		// Cap inbound batch; client must re-send remainder with a lower push cursor.
		const batch = changes.slice(0, MAX_SYNC_BATCH);
		...
		const run = db.transaction(() => {
			...
			for (const c of batch) {
				...
				if (ex && ex.updated_at >= c.updatedAt) {
					// Already have equal-or-newer: treat as accepted so client cursor can advance.
					if (c.updatedAt > acceptedMaxUpdatedAt) acceptedMaxUpdatedAt = c.updatedAt;
					acceptedCount += 1;
					continue;
				}
				seq += 1;
